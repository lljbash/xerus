...
void expose_leastSquaresAlgorithms(module& m) {
	...
	class_<TTRetractionI>(m,"TTRetractionI", init<const TTRetractionI &>());
	m.attr("ALSRetractionI") = object(TTRetractionI(&ALSRetractionI));
	m.attr("SubmanifoldRetractionI") = object(TTRetractionI(&SubmanifoldRetractionI));
	m.attr("HOSVDRetractionI") = object(TTRetractionI(&HOSVDRetractionI));

	class_<TTRetractionII>(m,"TTRetractionII", init<const TTRetractionII &>());
	m.attr("ALSRetractionII") = object(TTRetractionII(&ALSRetractionII));
	m.attr("SubmanifoldRetractionII") = object(TTRetractionII(&SubmanifoldRetractionII));
	m.attr("HOSVDRetractionII") = object(TTRetractionII(&HOSVDRetractionII));

	class_<TTVectorTransport>(m,"TTVectorTransport", init<const TTVectorTransport &>());
	m.attr("ProjectiveVectorTransport") = object(TTVectorTransport(&ProjectiveVectorTransport));

	{ scope als_scope =
		class_<ALSVariant>(m,"ALSVariant", init<uint, size_t, ALSVariant::LocalSolver, bool, optional<bool>>())
	...
	m.attr("ALS") = object(ptr(&ALS));
	m.attr("ALS_SPD") = object(ptr(&ALS_SPD));
	m.attr("DMRG") = object(ptr(&DMRG));
	m.attr("DMRG_SPD") = object(ptr(&DMRG_SPD));
	m.attr("ASD") = object(ptr(&ASD));
	m.attr("ASD_SPD") = object(ptr(&ASD_SPD));

	def("decomposition_als", &decomposition_als, (arg("x"), arg("b"), arg("epsilon")=EPSILON, arg("maxIterations")=1000));

	class_<GeometricCGVariant>(m,"GeometricCGVariant", init<size_t, value_t, bool, TTRetractionI, TTVectorTransport>())
	...
	;
	m.attr("GeometricCG") = object(ptr(&GeometricCG));

	class_<SteepestDescentVariant>(m,"SteepestDescentVariant", init<size_t, value_t, bool, TTRetractionII>())
	...
	;
	m.attr("SteepestDescent") = object(ptr(&SteepestDescent));
}
